    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="text-left">
                    <h2 className="text-4xl font-black text-gray-900 tracking-tighter uppercase italic leading-none">
                        {t('admin.business_lines.title').split(' ')[0]} <span className="text-blue-600 underline decoration-blue-100 decoration-8 underline-offset-4">{t('admin.business_lines.title').split(' ')[1]}</span>
                    </h2>
                    <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] mt-2">
                        {t('admin.business_lines.subtitle')}
                    </p>
                </div>
                <button
                    onClick={() => {
            ...
            <div className="relative max-w-xl text-left">
                ...
                <input
                    type="text"
                    placeholder={t('admin.business_lines.search_placeholder')}
                    value={searchTerm}
            ...
            <div className="grid grid-cols-1 gap-8">
                {(() => {
                    ...
                    if (loading) {
                        return <div className="py-24 text-center text-gray-300 font-black uppercase tracking-widest animate-pulse">{t('admin.business_lines.loading')}</div>;
                    }
                        ...
                        return (
                            <div className="bg-white rounded-[3.5rem] border-4 border-dashed border-gray-100 p-24 text-center space-y-6">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-black text-gray-900 uppercase italic">{t('admin.business_lines.empty_title')}</h3>
                                    <p className="text-gray-400 font-medium">{t('admin.business_lines.empty_desc')}</p>
                                </div>
                                {lines.length === 0 && (
                                    <button
                                    ...
                                    >
                                        {t('admin.business_lines.add_first')} <Plus size={16} />
                                    </button>
                                )}
                            </div>
                        );
                                            ...
                                            <div className="relative w-full h-full">
                                                ...
                                                {item.images.length > 1 && (
                                                    <div className="absolute bottom-3 right-3 bg-blue-600 text-white text-[10px] font-black px-2 py-1 rounded-lg shadow-lg border border-white/20">
                                                        +{item.images.length - 1} {t('admin.business_lines.table.gallery')}
                                                    </div>
                                                )}
                                            </div>
                                        ) : (
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex flex-wrap items-center gap-4">
                                                    <span className="text-[10px] font-black text-blue-600 bg-blue-50/50 px-3 py-1 rounded-lg uppercase tracking-widest italic">
                                                        {t('admin.business_lines.table.order')} {item.sort_order}
                                                    </span>
                                                    ...
                                                    <div className={`flex items-center gap-2 px-3 py-1 rounded-lg border ${item.is_active ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'
                                                        ...
                                                        <span className="text-[9px] font-black uppercase tracking-widest">{item.is_active ? t('admin.business_lines.status.active') : t('admin.business_lines.status.inactive')}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="flex gap-3">
                                                <button onClick={() => handleEdit(item)} title={t('common.edit')} className="p-4 text-gray-400 hover:text-blue-600 bg-white shadow-sm border border-gray-50 hover:border-blue-100 rounded-2xl transition-all">
                                                    <Edit2 size={20} />
                                                </button>
                                                <button onClick={() => handleDelete(item.id, item.title_id)} title={t('common.delete')} className="p-4 text-gray-400 hover:text-rose-500 bg-white shadow-sm border border-gray-50 hover:border-rose-100 rounded-2xl transition-all">
                                                    <Trash2 size={20} />
                                                </button>
                                            </div>
                                        </div>
                            ...
                            {totalPages > 1 && (
                                <div className="flex items-center justify-between p-4 px-8 bg-white rounded-[2rem] border border-gray-100">
                                    <div className="text-xs text-gray-400 font-bold uppercase tracking-widest">
                                        {t('common.showing')} {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredLines.length)} {t('common.of')} {filteredLines.length}
                                    </div>
                    ...
                    <div className={`bg-white shadow-2xl transition-all duration-500 flex flex-col animate-in zoom-in-95 ${isMaximized
                            ...
                            <div className="flex items-center gap-8 text-left">
                                ...
                                <div className="space-y-1">
                                    <h3 className="text-4xl font-black text-gray-900 uppercase tracking-tighter italic leading-none">
                                        {editingLine ? t('admin.business_lines.modal.edit_title') : t('admin.business_lines.modal.create_title')}
                                    </h3>
                                    <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">{t('admin.business_lines.modal.subtitle')}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <button
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-12 space-y-12 custom-scrollbar text-left">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black text-blue-600 uppercase tracking-[0.2em] px-4 italic leading-none flex items-center gap-2">
                                        <span className="w-6 h-1 bg-blue-600 rounded-full"></span> {t('admin.business_lines.form.title_id')}
                                    </label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center px-4">
                                        <label className="text-[11px] font-black text-emerald-500 uppercase tracking-[0.2em] italic leading-none flex items-center gap-2">
                                            <span className="w-6 h-1 bg-emerald-500 rounded-full"></span> {t('admin.business_lines.form.title_en')}
                                        </label>
                                        <button
                                            type="button"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] px-4 italic leading-none">{t('admin.business_lines.form.slug')}</label>
                                    <input
                                        type="text"
                                        value={formData.slug}
                                ...
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] px-4 italic leading-none">{t('admin.business_lines.form.sort')}</label>
                                    <input
                                        type="number"
                                ...
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] px-4 italic leading-none">{t('admin.business_lines.form.status')}</label>
                                    <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-[2.5rem] border-2 border-transparent focus-within:border-blue-500 transition-all">
                                        ...
                                        <span className="text-xs font-black uppercase tracking-widest text-gray-900">{formData.is_active ? t('admin.business_lines.form.status_active') : t('admin.business_lines.form.status_inactive')}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black text-blue-600 uppercase tracking-[0.2em] px-4 italic leading-none flex items-center gap-2">
                                        <span className="w-6 h-1 bg-blue-600 rounded-full"></span> {t('admin.business_lines.form.subtitle_id')}
                                    </label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center px-4">
                                        <label className="text-[11px] font-black text-emerald-500 uppercase tracking-[0.2em] italic leading-none flex items-center gap-2">
                                            <span className="w-6 h-1 bg-emerald-500 rounded-full"></span> {t('admin.business_lines.form.subtitle_en')}
                                        </label>
                                        <button
                                            type="button"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] px-4 italic leading-none">{t('admin.business_lines.form.color')}</label>
                                    <select
                                ...
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] px-4 italic leading-none">{t('admin.business_lines.form.icon')}</label>
                                    <select
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center justify-between px-4">
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] italic flex items-center gap-2">
                                        <span className="w-6 h-1 bg-gray-400 rounded-full"></span> {t('admin.business_lines.form.features_title')}
                                    </label>
                                    <button
                                        type="button"
                                    ...
                                    >
                                        <Plus size={14} /> {t('admin.business_lines.form.features_add')}
                                    </button>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    {formData.features.length === 0 && (
                                        <div className="md:col-span-2 py-12 text-center bg-gray-50 rounded-[3rem] border-4 border-dashed border-gray-100">
                                            <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">{t('admin.business_lines.form.features_empty')}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="space-y-8 pt-8 border-t border-gray-50">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h4 className="text-lg font-black text-gray-900 uppercase tracking-tighter italic">{t('admin.business_lines.form.visual_title')}</h4>
                                </div>
                                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                                        ...
                                        <div key={index} className="relative aspect-square bg-slate-900 rounded-[2rem] overflow-hidden border-4 border-white shadow-xl group/img">
                                            ...
                                            {index === 0 && (
                                                <div className="absolute top-3 left-3 bg-blue-600 text-white text-[9px] font-black px-3 py-1.5 rounded-lg uppercase tracking-widest shadow-2xl border border-white/20 italic">
                                                    {t('admin.business_lines.form.visual_primary')}
                                                </div>
                                            )}
                                        </div>
                                    ))}
                                    ...
                                    <div className="relative aspect-square">
                                        <FileUpload
                                            ...
                                            label={t('admin.business_lines.form.visual_add')}
                                            bucket="images"
                                            type="image"
                                            currentUrl=""
                                        />
                                ...
                                <p className="text-[10px] text-gray-300 font-bold uppercase tracking-widest italic px-4">
                                    {t('admin.business_lines.form.visual_info')}
                                </p>
                            </div>
                            ...
                            <div className="space-y-10 pt-10 border-t border-gray-50">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h4 className="text-lg font-black text-gray-900 uppercase tracking-tighter italic">{t('admin.business_lines.form.analysis_title')}</h4>
                                </div>
                                <div className={`grid gap-12 ${isMaximized ? 'grid-cols-2' : 'grid-cols-1'}`}>
                                    <div className="space-y-4">
                                        <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] px-6 italic flex items-center gap-3">
                                            ...
                                            {t('admin.business_lines.form.id_label')}
                                        </label>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center px-6">
                                            <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] italic flex items-center gap-3">
                                                ...
                                                {t('admin.business_lines.form.en_label')}
                                            </label>
                                            <button
                                                type="button"
                            ...
                            <div className="flex flex-col md:flex-row gap-6 pt-12 border-t border-gray-50">
                                ...
                                >
                                    <Save size={24} />
                                    {t('admin.business_lines.form.confirm_btn')}
                                </button>
                            </div>
                        </form>
                    </div>
