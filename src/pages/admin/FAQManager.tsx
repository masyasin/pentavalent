...
const FAQManager: React.FC = () => {
    ...
    const categories = [
        { id: 'general', label: t('admin.faq.cat.general') },
        { id: 'business', label: t('admin.faq.cat.business') },
        { id: 'investor', label: t('admin.faq.cat.investor') },
        { id: 'career', label: t('admin.faq.cat.career') },
        { id: 'other', label: t('admin.faq.cat.other') }
    ];
    ...
    return (
        <div className="space-y-6 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 text-left">
                <div>
                    <h2 className="text-3xl font-black text-gray-900 tracking-tighter italic">{t('admin.faq.title')}</h2>
                    <p className="text-gray-500 mt-1">{t('admin.faq.subtitle')}</p>
                </div>
                <button
                    onClick={() => {
                        resetForm();
            ...
            <div className="grid gap-4">
                {(() => {
                    ...
                    if (loading) {
                        return <div className="p-12 text-center text-gray-400 font-black uppercase tracking-widest animate-pulse">{t('admin.faq.loading')}</div>;
                    }
                    ...
                    if (filteredFaqs.length === 0) {
                        return (
                            <div className="bg-white rounded-[2rem] border border-gray-100 p-12 text-center">
                                ...
                                <h3 className="text-lg font-bold text-gray-900">{t('admin.faq.empty_title')}</h3>
                                <p className="text-gray-500">{t('admin.faq.empty_desc')}</p>
                            </div>
                        );
                    }
                    ...
                    return (
                        <>
                            {paginatedFaqs.map((faq) => (
                                <div key={faq.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:border-blue-200 transition-all group text-left">
                                        ...
                                        <div className="flex-1 space-y-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${faq.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                                    {faq.is_active ? t('common.active') : t('common.draft_label')}
                                                </span>
                                            </div>
                                ...
                                <div className="flex flex-col md:flex-row items-center justify-between p-4 px-8 bg-white rounded-[2rem] border border-gray-100 gap-4">
                                    <div className="flex items-center gap-4">
                                        <div className="text-xs text-gray-400 font-bold uppercase tracking-widest">
                                            {t('common.showing')} {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredFaqs.length)} {t('common.of')} {filteredFaqs.length}
                                        </div>
                                        <select
                                            value={itemsPerPage}
                                            onChange={(e) => {
                                        ...
                                        >
                                            <option value={6}>6 {t('common.per_page')}</option>
                                            <option value={12}>12 {t('common.per_page')}</option>
                                            <option value={24}>24 {t('common.per_page')}</option>
                                            <option value={48}>48 {t('common.per_page')}</option>
                                        </select>
                                    </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-0 md:p-4 transition-all duration-300">
                    <div className="bg-white rounded-[3rem] max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl flex flex-col scale-in-center">
                        <div className="p-10 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                            <div className="text-left">
                                <h3 className="text-3xl font-black text-gray-900 tracking-tighter italic">
                                    {editingFaq ? t('admin.faq.modal.edit_title') : t('admin.faq.modal.add_title')}
                                </h3>
                                <p className="text-gray-500 mt-1">{t('admin.faq.modal.subtitle')}</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-10 space-y-8 text-left">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">{t('admin.faq.form.category')}</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">{t('admin.faq.form.sort')}</label>
                                    <input
                                        type="number"
                            ...
                            <div className="grid lg:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">{t('admin.faq.form.question_id')}</label>
                                        <input
                                            type="text"
                                            required
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">{t('admin.faq.form.answer_id')}</label>
                                        <RichTextEditor
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center px-1">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">{t('admin.faq.form.question_en')}</label>
                                            <button
                                                type="button"
                                            ...
                                            >
                                                ...
                                                <span className="text-[8px] font-black uppercase tracking-widest leading-none">{t('common.auto_translate')}</span>
                                            </button>
                                        </div>
                                        <input
                                            type="text"
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center px-1">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">{t('admin.faq.form.answer_en')}</label>
                                            <button
                                                type="button"
                                                onClick={() => handleAutoTranslate(formData.answer_id || '', 'answer_en')}
                                                disabled={!!translating}
                                                className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-600 hover:text-white transition-all group"
                                            >
                            ...
                            <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl cursor-pointer" onClick={() => setFormData({ ...formData, is_active: !formData.is_active })}>
                                ...
                                <span className="font-bold text-gray-700 select-none">{t('admin.faq.form.publish')}</span>
                            </div>
                            ...
                            <div className="pt-6 flex gap-4">
                                ...
                                >
                                    ...
                                    {editingFaq ? t('admin.faq.form.save') : t('admin.faq.form.publish_btn')}
                                </button>
                            </div>
                        </form>
                    </div>
