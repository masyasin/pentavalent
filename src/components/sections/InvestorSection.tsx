  ...
  return (
    <section id="investors" className="py-24 md:py-48 bg-white relative overflow-hidden max-md:py-16 max-md:overflow-x-hidden">
      ...
      <div className="max-w-[1700px] mx-auto px-6 md:px-12 lg:px-16 relative z-10 max-md:px-4">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-20 gap-12 max-md:mb-10 max-md:gap-8">
          <div className="max-w-4xl max-md:w-full">
            <div className="inline-flex items-center gap-3 px-4 py-2 bg-slate-50 border border-slate-100 rounded-lg mb-8 max-md:mb-4 max-md:px-3 max-md:py-1.5">
              <div className="w-1.5 h-1.5 bg-blue-600 rounded-full max-md:shrink-0"></div>
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] max-md:text-[8px]">
          ...
          <div className="bg-slate-900 p-8 md:p-10 rounded-[2.5rem] shadow-2xl flex items-center gap-8 md:gap-10 group transition-all duration-500 overflow-hidden relative max-md:p-6 max-md:rounded-2xl max-md:gap-6">
            <div className="absolute inset-0 opacity-20 pointer-events-none max-md:hidden">
              <MiniStockChart symbol="PEVE" theme="dark" />
            </div>
            <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-white/10 flex items-center justify-center text-blue-400 group-hover:scale-110 transition-all border border-white/10 relative z-10 max-md:w-12 max-md:h-12 max-md:shrink-0">
              <svg className="w-8 h-8 md:w-10 md:h-10 max-md:w-6 max-md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
            </div>
            <div className="relative z-10">
              <div className="text-3xl md:text-4xl font-black text-white leading-none mb-2 tracking-tighter max-md:text-2xl">PEVE</div>
              <div className="text-[9px] md:text-[10px] font-black text-white/40 uppercase tracking-[0.4em] max-md:text-[8px]">{t('investor.ticker.label')}</div>
            </div>
          </div>
        </div>

        {/* Financial Highlights - Formalized */}
        <div className="grid md:grid-cols-3 gap-8 mb-20 max-md:grid-cols-1 max-md:gap-4 max-md:mb-10">
          {[
          ...
          <div className="bg-slate-900 rounded-[3.5rem] p-12 md:p-20 text-white relative overflow-hidden group max-md:p-8 max-md:rounded-[2rem]">
            ...
            <div className="space-y-4 max-md:space-y-3">
              {[
                ...
                <button key={i} className="w-full flex items-center justify-between p-8 bg-white/5 hover:bg-white/10 rounded-2xl border border-white/5 transition-all group/item max-md:p-4 max-md:min-h-[44px]">
                  <div className="text-left">
                    <div className="font-black tracking-tight text-lg mb-1 max-md:text-sm">{item.title}</div>
                    <div className="text-[10px] font-bold text-white/40 uppercase tracking-widest max-md:text-[8px]">{item.date}</div>
                  </div>
                  <div className="w-12 h-12 bg-white text-slate-900 rounded-xl flex items-center justify-center shadow-lg group-hover/item:bg-blue-500 group-hover/item:text-white transition-all max-md:w-8 max-md:h-8 max-md:shrink-0">
                    <svg className="w-5 h-5 max-md:w-4 max-md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1M7 10l5 5m0 0l5-5m-5 5V3" /></svg>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="bg-white rounded-[3.5rem] p-12 md:p-20 border border-slate-100 shadow-2xl relative max-md:p-8 max-md:rounded-[2rem]">
            ...
            <div className="space-y-10 max-md:space-y-6">
              {(calendar.length > 0 ? calendar : [
                ...
                <div key={i} className="flex items-center gap-10 group border-b border-slate-50 pb-10 last:border-0 last:pb-0 max-md:gap-4 max-md:pb-6">
                  <div className="text-center w-24 max-md:w-16">
                    <div className="text-sm font-black text-slate-400 uppercase tracking-widest mb-1 max-md:text-[10px]">
                      {new Date(event.event_date).toLocaleDateString('en-US', { month: 'short' }).toUpperCase()}
                    </div>
                    <div className="text-3xl font-black text-slate-900 max-md:text-xl">
                      {new Date(event.event_date).getDate()}
                    </div>
                  </div>
                  <div className="flex-1">
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 max-md:mb-1 max-md:text-[8px]">{event.event_type}</div>
                    <div className="text-xl font-bold text-slate-900 leading-tight group-hover:text-slate-600 transition-colors max-md:text-sm">
