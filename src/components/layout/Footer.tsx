  ...
  const social = [
    {
      ...
      link: '#',
      color: 'hover:wow-button-gradient hover:border-transparent'
    },
    {
      name: 'Instagram',
          ...
          <div className="pt-2 lg:pl-8">
            ...
            <ul className="space-y-4 pl-4 border-l border-white/5">
              {quickLinks.slice(0, 4).map((link) => (
                <li key={link.key}><button onClick={() => onNavigate(link.key)} className="text-slate-300 text-xs font-bold uppercase tracking-wide hover:text-white hover:translate-x-1 transition-all transition-transform duration-300 flex items-center gap-2 group text-left"><span className="w-0 overflow-hidden group-hover:w-2 transition-all duration-300 wow-text-primary">→</span>{link.label}</button></li>
              ))}
            </ul>
          </div>
