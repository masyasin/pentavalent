...
async function upgradeStrategiToEnterprise() {
    console.log('🚀 Upgrading Strategi Bisnis to Enterprise Level...');

    // 1. Get the Business Line ID
    ...
    const { error } = await supabase.from('business_advantages').insert(advantages);

    if (error) {
        console.error('Error upgrading advantages:', error.message);
    } else {
        console.log('✅ Enterprise strategy content seeded successfully!');
    }
}

upgradeStrategiToEnterprise();
